...
Bitcoin only & 689 & 346 & 343 & GRU & 0.784 & 0.789 & 0.816 & -0.0655 \\
All markets & 1235 & 650 & 585 & XGBoost & 0.777 & 0.791 & 0.853 & -0.0539 \\
Bitcoin only & 689 & 346 & 343 & XGBoost & 0.781 & 0.788 & 0.864 & -0.0626 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
The Bitcoin-only subset contains 689 markets, with 346 yes outcomes and 343 no outcomes, so it remains smaller than the full dataset but is no longer an extremely small corner case. Table~\ref{tab:bitcoin_only} shows that, at the standard 67\% setting, this more homogeneous market family makes the classification task slightly easier for both models. The GRU accuracy rises from 0.764 to 0.784, its F1 score rises from 0.776 to 0.789, and its AUC rises from 0.807 to 0.816. XGBoost also improves slightly in accuracy, from 0.777 to 0.781, and in AUC, from 0.853 to 0.864, while its F1 score is essentially unchanged at 0.788.

The more important pattern in Figure~\ref{fig:bitcoin_comparison} is that these classification gains do not translate into a better trading proxy. For both models, the average margin is more negative on the Bitcoin-only subset at every visibility ratio. The GRU reaches 0.959 accuracy and 0.981 AUC at 95\% visible history, but its average margin falls to -0.0730 dollars per trade, which is the worst margin in the Bitcoin-only sweep. XGBoost follows the same pattern: its later-ratio accuracy and AUC remain competitive, yet its average margin stays between -0.0446 and -0.0689 dollars per trade throughout the experiment. Therefore, the Bitcoin-only experiment supports a narrower conclusion. Restricting the dataset to Bitcoin-related markets makes the labels somewhat easier to predict, especially for the sequential model, but it does not improve the practical trading outcome. In this setting, greater domain homogeneity sharpens prediction quality while moving the strategy further away from profitability.
