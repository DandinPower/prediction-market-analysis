---
url: "https://docs.polymarket.com/cn/builders/overview"
title: "构建者计划 - Polymarket Documentation"
---

[跳转到主要内容](https://docs.polymarket.com/cn/builders/overview#content-area)

[Polymarket Documentation home page![light logo](https://mintcdn.com/polymarket-292d1b1b/ZibMe6Tli_amZald/logo/light.svg?fit=max&auto=format&n=ZibMe6Tli_amZald&q=85&s=9f9bd559f02181495794661ef6426c0f)![dark logo](https://mintcdn.com/polymarket-292d1b1b/ZibMe6Tli_amZald/logo/dark.svg?fit=max&auto=format&n=ZibMe6Tli_amZald&q=85&s=13abc6754579a3475368ea99d0f5a241)](https://docs.polymarket.com/cn)

![CN](https://d3gk2c5xim1je2.cloudfront.net/flags/CN.svg)

简体中文

搜索...

Ctrl K询问AI

搜索...

Navigation

构建者计划

构建者计划

[文档](https://docs.polymarket.com/cn) [API 参考](https://docs.polymarket.com/cn/api-reference/introduction)

![https://raw.githubusercontent.com/suhailkakar/demo/refs/heads/main/book.svg](https://raw.githubusercontent.com/suhailkakar/demo/refs/heads/main/book.svg)

##### 入门指南

- [概览](https://docs.polymarket.com/cn)
- [Polymarket 101](https://docs.polymarket.com/cn/polymarket-101)
- [快速入门](https://docs.polymarket.com/cn/quickstart)

![https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/layers.svg](https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/layers.svg)

##### 核心概念

- [市场与事件](https://docs.polymarket.com/cn/concepts/markets-events)
- [价格与订单簿](https://docs.polymarket.com/cn/concepts/prices-orderbook)
- [持仓与代币](https://docs.polymarket.com/cn/concepts/positions-tokens)
- [订单生命周期](https://docs.polymarket.com/cn/concepts/order-lifecycle)
- [判定](https://docs.polymarket.com/cn/concepts/resolution)

![https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/paper.svg](https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/paper.svg)

##### 市场数据

- [概览](https://docs.polymarket.com/cn/market-data/overview)
- [获取市场数据](https://docs.polymarket.com/cn/market-data/fetching-markets)
- [Subgraph](https://docs.polymarket.com/cn/market-data/subgraph)

![https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/chart.svg](https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/chart.svg)

##### 交易

- [概述](https://docs.polymarket.com/cn/trading/overview)
- [快速开始](https://docs.polymarket.com/cn/trading/quickstart)
- [Orderbook](https://docs.polymarket.com/cn/trading/orderbook)
- 订单

- [费用](https://docs.polymarket.com/cn/trading/fees)
- [免 Gas 交易](https://docs.polymarket.com/cn/trading/gasless)
- [Negative Risk 市场](https://docs.polymarket.com/cn/advanced/neg-risk)
- [撮合引擎重启](https://docs.polymarket.com/cn/trading/matching-engine)
- CTF 代币

- WebSocket

- 跨链桥


![https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/histogram.svg](https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/histogram.svg)

##### 做市商

- [概述](https://docs.polymarket.com/cn/market-makers/overview)
- [入门指南](https://docs.polymarket.com/cn/market-makers/getting-started)
- [Maker 返利计划](https://docs.polymarket.com/cn/market-makers/maker-rebates)
- [流动性奖励](https://docs.polymarket.com/cn/market-makers/liquidity-rewards)
- 操作


![https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/trophy.svg](https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/trophy.svg)

##### 构建者计划

- [构建者计划](https://docs.polymarket.com/cn/builders/overview)
- [API Keys](https://docs.polymarket.com/cn/builders/api-keys)
- [Tiers](https://docs.polymarket.com/cn/builders/tiers)

![https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/book-search.svg](https://mintlify.s3.us-west-1.amazonaws.com/polymarket-292d1b1b/images/icons/nav/book-search.svg)

##### 资源

- [合约地址](https://docs.polymarket.com/cn/resources/contract-addresses)
- [错误码](https://docs.polymarket.com/cn/resources/error-codes)

在此页面

- [计划权益](https://docs.polymarket.com/cn/builders/overview#%E8%AE%A1%E5%88%92%E6%9D%83%E7%9B%8A)
- [你将获得](https://docs.polymarket.com/cn/builders/overview#%E4%BD%A0%E5%B0%86%E8%8E%B7%E5%BE%97)
- [工作原理](https://docs.polymarket.com/cn/builders/overview#%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86)
- [开始使用](https://docs.polymarket.com/cn/builders/overview#%E5%BC%80%E5%A7%8B%E4%BD%BF%E7%94%A8)
- [SDK 与库](https://docs.polymarket.com/cn/builders/overview#sdk-%E4%B8%8E%E5%BA%93)
- [示例](https://docs.polymarket.com/cn/builders/overview#%E7%A4%BA%E4%BE%8B)
- [Safe 钱包示例](https://docs.polymarket.com/cn/builders/overview#safe-%E9%92%B1%E5%8C%85%E7%A4%BA%E4%BE%8B)
- [Proxy 钱包示例](https://docs.polymarket.com/cn/builders/overview#proxy-%E9%92%B1%E5%8C%85%E7%A4%BA%E4%BE%8B)
- [每个演示涵盖的内容](https://docs.polymarket.com/cn/builders/overview#%E6%AF%8F%E4%B8%AA%E6%BC%94%E7%A4%BA%E6%B6%B5%E7%9B%96%E7%9A%84%E5%86%85%E5%AE%B9)
- [下一步](https://docs.polymarket.com/cn/builders/overview#%E4%B8%8B%E4%B8%80%E6%AD%A5)

**构建者** 是指将用户订单路由到 Polymarket 的个人、团体或组织。如果你创建了一个允许用户通过你的系统在 Polymarket 上交易的平台,这个计划就是为你准备的。

## [​](https://docs.polymarket.com/cn/builders/overview\#%E8%AE%A1%E5%88%92%E6%9D%83%E7%9B%8A)  计划权益

## 免 Gas 交易

通过我们的 Relayer,所有链上操作都无需 Gas 费

## 订单归因

获得订单归属,在构建者排行榜上竞争奖励

## 收益分成

从你路由的订单中赚取手续费分成

### [​](https://docs.polymarket.com/cn/builders/overview\#%E4%BD%A0%E5%B0%86%E8%8E%B7%E5%BE%97)  你将获得

| 权益 | 说明 |
| --- | --- |
| **Relayer 访问权限** | 免 Gas 的钱包部署、授权、订单执行和 CTF 操作 |
| **交易量追踪** | 所有订单都归属到你的构建者资料 |
| **每周奖励** | 基于交易量的 USDC 奖励计划(已认证+) |
| **排行榜** | 在 [builders.polymarket.com](https://builders.polymarket.com/) 上公开展示 |
| **技术支持** | Telegram 频道和工程支持(已认证+) |

EOA 钱包没有 Relayer 访问权限。直接从 EOA 交易的用户需要自己支付 Gas 费。

## [​](https://docs.polymarket.com/cn/builders/overview\#%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86)  工作原理

1

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

用户下单

用户通过你的应用程序下单。

2

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

签署请求

你的应用使用 Builder API 凭证签署请求。

3

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

提交到 CLOB

订单被提交到 Polymarket 的 CLOB,并带有归因头信息。

4

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

执行交易

Polymarket 撮合订单并承担链上操作的 Gas 费。

5

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

交易量归因

交易量被计入你的构建者账户。

## [​](https://docs.polymarket.com/cn/builders/overview\#%E5%BC%80%E5%A7%8B%E4%BD%BF%E7%94%A8)  开始使用

1

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

创建构建者资料

前往
[polymarket.com/settings?tab=builder](https://polymarket.com/settings?tab=builder)
生成你的 API 密钥。

2

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

配置归因

设置你的 CLOB 客户端,在每个订单中包含构建者认证头信息。

3

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

启用免 Gas 交易

使用 Relayer Client 实现免 Gas 的钱包部署和链上操作。

4

[Navigate to header](https://docs.polymarket.com/cn/builders/overview#)

跟踪表现

在 [构建者排行榜](https://builders.polymarket.com/) 上监控你的交易量。

## [​](https://docs.polymarket.com/cn/builders/overview\#sdk-%E4%B8%8E%E5%BA%93)  SDK 与库

[**CLOB Client (TypeScript)** \\
\\
下单并进行构建者归因](https://github.com/Polymarket/clob-client)

[**CLOB Client (Python)** \\
\\
下单并进行构建者归因](https://github.com/Polymarket/py-clob-client)

[**Relayer Client (TypeScript)** \\
\\
免 Gas 的链上交易](https://github.com/Polymarket/builder-relayer-client)

[**Relayer Client (Python)** \\
\\
免 Gas 的链上交易](https://github.com/Polymarket/py-builder-relayer-client)

[**CLOB Client (Rust)** \\
\\
下单并进行构建者归因](https://github.com/Polymarket/rs-clob-client)

[**Signing SDK (TypeScript)** \\
\\
签署构建者认证头信息](https://github.com/Polymarket/builder-signing-sdk)

[**Signing SDK (Python)** \\
\\
签署构建者认证头信息](https://github.com/Polymarket/py-builder-signing-sdk)

## [​](https://docs.polymarket.com/cn/builders/overview\#%E7%A4%BA%E4%BE%8B)  示例

这些开源演示应用展示了如何集成 Polymarket 的 CLOB Client 和 Builder Relayer Client,实现带构建者订单归因的免 Gas 交易。

## 身份认证

支持多种钱包提供商

## 免 Gas 交易

支持 Safe 和 Proxy 钱包

## 完整集成

订单、仓位、CTF 操作

### [​](https://docs.polymarket.com/cn/builders/overview\#safe-%E9%92%B1%E5%8C%85%E7%A4%BA%E4%BE%8B)  Safe 钱包示例

为你的用户部署 Gnosis Safe 钱包:

[**wagmi + Safe** \\
\\
MetaMask、Phantom、Rabby 和其他浏览器钱包](https://github.com/Polymarket/wagmi-safe-builder-example)

[**Privy + Safe** \\
\\
Privy 嵌入式钱包](https://github.com/Polymarket/privy-safe-builder-example)

[**Magic Link + Safe** \\
\\
Magic Link 邮箱/社交登录认证](https://github.com/Polymarket/magic-safe-builder-example)

[**Turnkey + Safe** \\
\\
Turnkey 嵌入式钱包](https://github.com/Polymarket/turnkey-safe-builder-example)

### [​](https://docs.polymarket.com/cn/builders/overview\#proxy-%E9%92%B1%E5%8C%85%E7%A4%BA%E4%BE%8B)  Proxy 钱包示例

针对来自 Polymarket.com 的现有 Magic Link 用户:

[**Magic Link + Proxy** \\
\\
为 Polymarket.com Magic 用户自动部署 Proxy 钱包](https://github.com/Polymarket/magic-proxy-builder-example)

### [​](https://docs.polymarket.com/cn/builders/overview\#%E6%AF%8F%E4%B8%AA%E6%BC%94%E7%A4%BA%E6%B6%B5%E7%9B%96%E7%9A%84%E5%86%85%E5%AE%B9)  每个演示涵盖的内容

- 身份认证

- 钱包操作

- 交易


- 通过钱包提供商进行用户登录
- 用户 API 凭证派生(L2 认证)
- 使用远程签名配置构建者
- Safe 和 Proxy 钱包的签名类型

- 通过 Relayer 部署 Safe 钱包
- 批量代币授权(USDC.e + 结果代币)
- CTF 操作(拆分、合并、兑换)
- 交易监控

- CLOB 客户端初始化
- 带构建者归因的订单下单
- 仓位和订单管理
- 通过 Gamma API 发现市场

* * *

## [​](https://docs.polymarket.com/cn/builders/overview\#%E4%B8%8B%E4%B8%80%E6%AD%A5)  下一步

[**获取 API 密钥** \\
\\
创建和管理你的 Builder API 凭证。](https://docs.polymarket.com/builders/api-keys)

[**了解等级** \\
\\
了解速率限制以及如何升级。](https://docs.polymarket.com/builders/tiers)

[**订单归因** \\
\\
配置你的客户端,将交易归属到你的账户。](https://docs.polymarket.com/trading/orders/attribution)

[**免 Gas 指南** \\
\\
为你的用户设置免 Gas 交易。](https://docs.polymarket.com/trading/gasless)

此页面对您有帮助吗？

是否

[库存管理\\
\\
上一页](https://docs.polymarket.com/cn/market-makers/inventory) [API Keys\\
\\
下一页](https://docs.polymarket.com/cn/builders/api-keys)

Ctrl+I

助手

AI生成的回答可能包含错误。