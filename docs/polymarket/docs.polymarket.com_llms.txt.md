...
\- \[Get market prices (request body)\](https://docs.polymarket.com/api-reference/market-data/get-market-prices-request-body.md): Retrieves market prices for multiple token IDs and sides using a request body. Each request must include both token\_id and side.
...
\- \[Get all relayer API keys\](https://docs.polymarket.com/api-reference/relayer-api-keys/get-all-relayer-api-keys.md): Returns all relayer API keys for the authenticated address. Auth allowed: Gamma auth or Relayer API key auth (\`RELAYER\_API\_KEY\` + \`RELAYER\_API\_KEY\_ADDRESS\`).
